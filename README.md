...
![](https://img.shields.io/badge/version-2.2.2-blue)
...
从 [Releases](https://github.com/BFRKQSB7/balance-hud/releases) 下载 `balance-hud-v2.2.2.zip`：
...
### v2.2.2
- **优化**：标签颜色默认从 `dim`（灰色）改为 `white`（亮白）— 黑色终端下时长/速度/用量/token 等标签更清晰可读
- **新增**：颜色值支持 `white`（`\x1b[97m`），`colors.label` 可配置 `"white"`，校验器同步接受
